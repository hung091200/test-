<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %><!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
</head>
<body>
<div>
    <h2>Thêm nhân viên</h2>
    <form action="/nhan-vien/add" method="post">
        STT: <input type="number" name="stt"><br>
        Họ tên: <input type="text" name="hoten"><br>
        Tuổi: <input type="number" name="tuoi"><br>
        SĐT: <input type="text" name="sdt"><br>
        Giới tính: <input type="text" name="gioi_tinh"><br>
        Phòng ban: <input type="text" name="phong_ban"><br>
        Vị trí: <input type="text" name="vi_tri"><br>
        <button type="submit">Lưu</button>
    </form>
</div>

<h2>Danh sách nhân viên</h2>
<table>
    <thead>
    <tr>
        <th>mã nhân viên</th>
        <th>họ và tên</th>
        <th>tuổi</th>
        <th>Số điện thoại</th>
        <th>Giới tính</th>
        <th>Phòng ban</th>
        <th>Vị trí</th>
        <th>Thao </th>
    </tr>
    </thead>

    <tbody>
    <c:forEach var="item" items="${nhanVien}">
        <tr>
<%--            <td>${loop
}</td>--%>
            <td>${item.stt}</td>
            <td>${item.hoten}</td>
            <td>${item.tuoi}</td>
            <td>${item.sdt}</td>
            <td>${item.gioi_tinh}</td>
            <td>${item.phong_ban}</td>
            <td>${item.vi_tri}</td>
            <td>
                <a href="/nhan-vien/update/${item.stt}">Detail</a>
                <a href="/nhan-vien/delete/${item.stt}">Delete</a>
            </td>
        </tr>
    </c:forEach>
    </tbody>
</table>
</body>
</html>